...
rpath=os.path.join(base_folder,'S03','*.mov')
all_path=glob(rpath)
h=1
import shutil
...
# for subject_number, bb_start in subject_list:
    ...
    rpath, fname=os.path.split(dpath)
    sfolder=os.path.join(rpath,'processed_video')
    check_make_folder(sfolder)
    spath=os.path.join(sfolder,'process.mp4')
    h=1
    # full_dir_output = os.path.join(base_folder, output_vid_name)
    # vid_name = 'test_video.mp4'
    # vid_name = 'S03_MD.mov'
    # capturing video from input file
    cap = cv2.VideoCapture(dpath)
    # print(input_vid_name)
    # logging.info(f'process {input_vid_name}')
